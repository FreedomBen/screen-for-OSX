...
fd_set *rp, *wp;
{
  ...
  for (display = displays; display; display = display->_d_next)
      ...
      if (d_fore == 0)
	{
      ...
      if (d_pastelen)
        {
	  /* paste to win/pseudo */
	  FD_SET(W_UWP(d_fore) ? d_fore->w_pwin->p_ptyfd : d_fore->w_ptyfd, wp);
	}
      else
#endif
  ...
  for (p = windows; p; p = p->w_next)
	...
	{
	  debug2("mkfdsets: want to write %d bytes to pseudo (#%d)\n", 
		 p->w_pwin->p_inlen, p->w_number);
      ...
      if (p->w_active && (d_obufp - d_obuf) > d_obufmax)
	{
      ...
      if (p->w_pwin && (p->w_pwin->p_inlen >= sizeof(p->w_pwin->p_inbuf)))
	{
	...
	{
	  /*
	  debug1("mkfdsets: may read from pseudo (#%d)\n", p->w_number);
	   */
...
# endif /* UTMPOK */
  ...
  for (;;)
      ...
      for (display = displays; display; display = display->_d_next)
	{
	  ...
	  time_left = d_status_time + (d_status_bell ? VBellWait : MsgWait) - time((time_t *)0);
	  if (time_left > 0)
	    {
      ...
      if (nsel)
	{
	  for (display = displays; display; display = display->_d_next)
	    {
	      if ((fore = d_fore) == 0 || d_pastelen == 0)
		continue;
	      /* pasting is just like user input */
	      tmp = W_UWP(fore) ? fore->w_pwin->p_ptyfd : fore->w_ptyfd;
	      if (! FD_ISSET(tmp, &w))
		continue;
	      debug1("writing pastebuffer (%d)", d_pastelen);
	      debug1("%s\n", W_UWP(fore) ? " (pseudowin)" : "");
	      len = write(tmp, d_pastebuffer, d_pastelen > IOSIZE ? IOSIZE : d_pastelen);
	  ...
	  for (p = windows; p; p = p->w_next)
	    {
	      if (p->w_pwin && p->w_pwin->p_inlen > 0)
	        {
#ifdef COPY_PASTE
		  if (p->w_display && p->w_display->_d_pastelen)
		    continue;
#endif
		  /* stuff p_inbuf into pseudowin */
...
# endif
	  ...
	  /*
	   * O.k. All streams are fed, now look what comes back to us.
	   * First of all: user input.
	   */
      ...
      for (p = windows; p; p = p->w_next) 
	{
	  ...
	  if (p->w_outlen)
	  ...
	  else 
	    {
	      ...
	      while (W_RP(p) && nsel && FD_ISSET(p->w_pwin->p_ptyfd, &r))
	        {
		  nsel--;
		  n = 0;
		  if ((((p->w_pwin->fdpat >> F_PSHIFT) & F_PBOTH) == F_PBOTH) ||
		      (((p->w_pwin->fdpat >> (F_PSHIFT * 2)) & F_PBOTH) == F_PBOTH))
		    {
		      /* Window wants a copy of the pseudowin output */
		      /* pseudo read select with full buffer must not happen */
		      if (!(tmp = sizeof(p->w_inbuf) - p->w_inlen))
		        ASSERT(0);
		      n++;
		    }
		  else
...
/* HERE WE ASSUME THAT THERE IS NO PACKET MODE ON PSEUDOWINS */
	      ...
	      if (nsel && FD_ISSET(p->w_ptyfd, &r))
		{
		  ...
		  n = 0;
		  ASSERT(W_RW(p));
		  if (p->w_pwin && ((p->w_pwin->fdpat & F_PBOTH) == F_PBOTH))
		    {
		      /* 
		       * Our pseudowin wants a copy of the window output, 
		       * so don't read to much for its (full) buffer.
		       */
		      if (!(tmp = sizeof(p->w_pwin->p_inbuf) - p->w_pwin->p_inlen))
			ASSERT(0);
		      n++;
		    }
		  else
