...
# else
#  if defined(hpux) || defined(_SEQUENT_) || defined(SVR4)
...
char *fn, **av;
{
  register int flowflag, num, lflag = loginflag;
...
int dump;
{
  ...
  if (UserContext() > 0)
	...
	{
	  debug1("WriteFile: fopen(%s) failed\n", fn);
	  UserReturn(0);
	}
      else
	{
...
ReInitUtmp()
{
  ...
  loginslot = TtyNameSlot(ttyname(0));
...
#ifdef NETHACK
          if (nethackflag)
            Msg(errno, "/etc/utmp is to hard to dig");
	  else
#endif
          Msg(errno,"Could not write /etc/utmp");
        }
    }
  loginslot = (slot_t) 0;
}
...
# endif
	...
	{
	  DeadlyMsg = 0;
	  Msg(errno, "cannot read /etc/utmp ???");
	  sleep(1);
	}
      (void) lseek(utmpf, (off_t) (slot * sizeof u), 0);
...
# endif
  if (loginslot != (slot_t)0 && loginslot != (slot_t)-1 && host[0] != '\0')
    ...
    {
      strncpy(host + 1, stripdev(ttyname(0)), sizeof(host) - 6);
      host[0] = ':';
    }
