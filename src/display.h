...
struct display
{
  ...
  char	_d_usertty[MAXPATH];	/* tty we are attached to */
  ...
  int	_d_pastelen;
#endif
  union	tc _d_tcs[T_N];		/* terminal capabilities */
...
#define d_copybuffer	DISPLAY(_d_copybuffer)
#define d_copylen	DISPLAY(_d_copylen)
#define d_pastebuffer	DISPLAY(_d_pastebuffer)
#define d_pastelen	DISPLAY(_d_pastelen)
